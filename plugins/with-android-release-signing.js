...
const RELEASE_SIGNING_CONFIG = `
        release {
            if (liftlogReleaseBuild) {
                storeFile = file(System.getenv("LIFTLOG_ANDROID_KEYSTORE_PATH"))
...
module.exports = function withAndroidReleaseSigning(config) {
  return withAppBuildGradle(config, configMod => {
     ...
     * buildTypes {
     *
     * Insert our release signing config before signingConfigs closes.
     */
    ...
    /*
     * Expo normally signs release builds with the debug signing config.
     *
     * Replace:
     *
     * signingConfig signingConfigs.debug
     *
     * inside buildTypes.release with:
     *
    ...
    /*
     * Add release validation before the android block.
     */
    ...
    /*
     * Preserve the versionCode generated by Expo as the fallback for
     * normal development builds.
     *
     * For release builds the Gradle property is mandatory, so the guard
     * above guarantees this value exists.
     */
    const versionCodePattern = /versionCode\s+(\d+)/;

    const versionCodeMatch = contents.match(versionCodePattern);
    ...
    const configuredVersionCode = versionCodeMatch[1];

    contents = contents.replace(
      versionCodePattern,
      `versionCode = Integer.parseInt((liftlogAndroidVersionCode ?: '${configuredVersionCode}').toString())`
    );
