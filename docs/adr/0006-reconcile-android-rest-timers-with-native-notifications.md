# ADR-0006: Reconcile Android rest timers with native notifications
...
Android rest timer completion must remain timely when LiftLog is backgrounded,
locked, or restarted without producing duplicate feedback while the app is
active. A JavaScript interval cannot provide background delivery, while a native
notification alone cannot own timer state or guarantee exact delivery without
additional system access. iOS has no physical-device validation path in this
project, so native rest timer notification delivery remains outside its scope.
...
- Persist a minimal, versioned runtime snapshot in a dedicated MMKV namespace.
  ...
  backups.
- Use foreground completion feedback on both platforms while LiftLog is active.
  On Android, use one native local notification while LiftLog is inactive,
  backgrounded, locked, or terminated. Do not add iOS native rest timer delivery.
- Cancel pending delivery for pause, manual cancellation, replacement, invalid
  ...
  notification.
- Store an Android-only opt-in rest timer notification preference with user-owned
  settings and include it in backup schema version 2. Migrate version-1 backups
  with the preference disabled. Never back up or restore OS authorization.
...
- Use LiftLog's short completion sound for Android native delivery and leave final
  delivery control to Android notification and channel settings.
...
- iOS must remain build-compatible, but only foreground completion feedback is
  supported there until physical-device validation becomes available.
